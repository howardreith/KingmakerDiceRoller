...
from domain_reference import Assignment, DiceError, Parser, RolledArray, SequenceRandom, point_buy, score_cost
...
class DomainReferenceTests(unittest.TestCase):
    ...
    def test_nested_keep_count(self): self.assertEqual(9, Parser().parse('4d[6]kh(1d[2]+1)').evaluate(SequenceRandom([6,5,4,1,1])))
